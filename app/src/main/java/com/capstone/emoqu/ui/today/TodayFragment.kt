    ...
    private fun getActivityData() {
        viewModel.getActivityData().observe(viewLifecycleOwner) { result ->
            when (result) {
                ...
                is Result.Success -> {
                    binding.progressBar.visibility = View.GONE
                    val activitiesToday = result.data.activities.filter {
                        isToday(it.timeStamp)
                    }

                    Log.d("TodayFragment", "Activities size for today: ${activitiesToday.size}")
                    activitiesToday.forEach { Log.d("TodayFragment", it.toString()) }

                    if (activitiesToday.isEmpty()) {
                        binding.viewSwitcher.displayedChild = 0 // Show the ImageView
                        Toast.makeText(
                            requireContext(),
                            "You don't have any activity today, please add activity data first(:",
                            Toast.LENGTH_SHORT
                        ).show()
                    } else {
                        binding.viewSwitcher.displayedChild = 1 // Show the RecyclerView
                        activityAdapter.submitList(activitiesToday)
                        Toast.makeText(
                            requireContext(),
